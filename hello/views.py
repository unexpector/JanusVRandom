...
def index(request):
    ...
    thecube.setcords(3,1,4)


    themodel = ObjectLibrary.objects.get(objectname=random_model).src
    context_dict = {'boldmessage': thecube.name, 'randomreddit': randomreddit, 'randomwikipedia': randomwikipedia, 'thenumber': thenumber, 'themodel': themodel, 'wikipediaid': wikipedia_id, 'realrandom': fullrandom, 'randomroom': random_room, 'xvalue': x_dimension, 'yvalue': y_dimension, 'zvalue': z_dimension, 'newobjx': newy }
